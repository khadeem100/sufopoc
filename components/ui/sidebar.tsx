...
export const DesktopSidebar = ({
  ...
  return (
    <motion.div
      className={cn(
        "h-screen px-4 py-4 hidden md:flex md:flex-col bg-white border-r border-gray-200 w-[300px] flex-shrink-0",
        className
      )}
      animate={{
...
export const SidebarLink = ({
  ...
  const handleClick = (e: React.MouseEvent<HTMLAnchorElement>) => {
    ...
    // Handle logout
    if (link.href === "/auth/signout" || link.label.toLowerCase() === "logout") {
      e.preventDefault();
      signOut({ callbackUrl: "/" });
      return;
    }
